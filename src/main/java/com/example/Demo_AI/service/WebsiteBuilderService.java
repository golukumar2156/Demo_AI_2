...
import com.example.Demo_AI.aiTools.wesite.WebsiteTools;
...
public class WebsiteBuilderService {
    ...
    public String generate(String message) {
     ...
     //Website generate karta hai aur poore project folder ko ZIP bytes ke roop me return karta hai.
    public byte[] generateZip(String message) throws IOException {
        generate(message); // LLM tools ke through files bana dega

        String projectRoot = websiteTools.getLastProjectRoot();
        if (projectRoot == null) {
            throw new IllegalStateException("No project was created for this request.");
        }

        return websiteTools.zipProject(projectRoot);
    }
}